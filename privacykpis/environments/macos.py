...
TRUSTED_SHELL_KARGS = dict(shell=True, check=True, capture_output=True, text=True)
...
def setup_env(args: Args):
  ...
  configure_proxy_cmds = [
    ["-setwebproxy", default_net_service, args.proxy_host, args.proxy_port],
    ["-setwebproxystate", default_net_service, "on"],
    ["-setsecurewebproxy", default_net_service, args.proxy_host, args.proxy_port],
  ...
  trust_cert_cmd = ["sudo", "security", "add-trusted-cert", "-d", "-r",
...
def teardown_env(args: Args):
  ...
  configure_proxy_cmds = [
    ...
    ["-setproxybypassdomains", default_net_service, "*.local", "169.254/16"],
  ]

  for a_cmd in configure_proxy_cmds:
    subprocess.run(SUDO_NETWORK_STUB + a_cmd, check=True)

  untrust_cert_cmd = ["sudo", "security", "delete-cert", "-c",
    "mitmproxy", SYS_KEYCHAIN]
