...
from privacykpis.consts import DEFAULT_PROFILE_PATH, DEFAULT_LOG_PATH
...
PARSER.add_argument("--profile-template", default=None,
                    help="Nonstandard (i.e. including extensions) profile to "
                         "use. Only works when the directory pointed to by "
                         "--profile-path is empty. Should be path relative "
...
if ARGS.url:
    ...
    while True:
        ...
        if hasattr(ARGS, "subcase"):
            ...
            treatment = ARGS.case
        packed = conn.blpop([f'queue:{treatment}:{ARGS.profile_index}'], 30)
        if not packed:
            continue
